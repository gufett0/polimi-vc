...
const BUILD_DIR = path.join(__dirname, "../build");
const OUTPUT_DIR = path.join(__dirname, "../proofs");
const TEST_EMAIL_PATH = path.join(__dirname, "./emls/email-test.eml");

describe("Verifier Circuit Test with Proof Generation", () => {
  jest.setTimeout(2 * 60 * 60 * 1000); // Extend Jest timeout globally

  let rawEmail: Buffer;
  let circuit: any;

  beforeAll(async () => {
    rawEmail = fs.readFileSync(TEST_EMAIL_PATH, "utf8");
  ...
  it("should generate circuit inputs", async () => {
    console.time("Circuit Input Generation");
    const ethereumAddress = "0x3773fd7b2a9CF6FF3E3EfD82Bd30536E11e83b6f";
    const circuitInputs = await generateVerifierCircuitInputs(rawEmail, ethereumAddress);
    console.timeEnd("Circuit Input Generation");
  });

  it("should calculate witness", async () => {
    const circuitInputs = await generateVerifierCircuitInputs(rawEmail, "0x3773fd7b2a9CF6FF3E3EfD82Bd30536E11e83b6f");
  ...
  it("should check constraints", async () => {
    const circuitInputs = await generateVerifierCircuitInputs(rawEmail, "0x3773fd7b2a9CF6FF3E3EfD82Bd30536E11e83b6f");
    const witness = await circuit.calculateWitness(circuitInputs);
    console.time("Constraints Checking");
    await circuit.checkConstraints(witness);
    console.timeEnd("Constraints Checking");
  });

  it("should verify DKIM", async () => {
    console.time("DKIM Verification");
    const dkimResult = await verifyDKIMSignature(rawEmail, "polimi.it");
    console.timeEnd("DKIM Verification");
  });
  ...
  it("should generate and verify proof", async () => {
    ...
    const wasm = fs.readFileSync(path.join(BUILD_DIR, `${CIRCUIT_NAME}_js/${CIRCUIT_NAME}.wasm`));
    const wc = require(path.join(BUILD_DIR, `${CIRCUIT_NAME}_js/witness_calculator.js`));
    const witnessCalculator = await wc(wasm);
    const wtnsBuffer = await witnessCalculator.calculateWTNSBin(circuitInputs, 0);
    fs.writeFileSync(path.join(OUTPUT_DIR, `input.wtns`), wtnsBuffer);
    ...
    fs.writeFileSync(path.join(OUTPUT_DIR, `proof.json`), JSON.stringify(proof, null, 2));
    fs.writeFileSync(path.join(OUTPUT_DIR, `public.json`), JSON.stringify(publicSignals, null, 2));
