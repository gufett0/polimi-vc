...
describe("Verifier Contract", function () {
  ...
  //   console.log("verifier.sol deployed at: ",verifier.target)
  //   console.log(verifier.getDeployTransaction)
  // });
  ...
  it("Should verify the proof using actual proof.json and public.json", async function () {
    ...
    verifier = await ethers.getContractAt("Verifier", verifier_address);
    ...
    // Call the verifier's verifyProof function with the formatted inputs
    const result = await verifier.verifyProof(proof_a, proof_b, proof_c, input, {gasLimit: 500000 });
